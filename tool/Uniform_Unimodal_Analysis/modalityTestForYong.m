...
% 结果只输出符合的mode（p>0.05）的p值和mode，p<0.05的不输出
...
for j=1:nm
	...
	p=U2bootstrap-U2N>0;
	pval(1,j)=sum(p)/Nb;  % edited by Lwh 20180615
%     pval(j) = sum(p)/Nb; % edited by Lwh 20180615
	
	if pval(1,j)>0.05 % edited by Lwh 20180615
%     if pval(j)>0.05 % edited by Lwh 20180615
        if mode==0
            mode=modelist(j);
        end
	end
